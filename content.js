...
(async () => {
  ...
  widget.innerHTML = `
    ...
    <div class="nav-tabs">
      ...
      <button class="tab-btn" data-tab="h2r">
        <svg style="width:18px;height:18px" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
    ...
    <div class="tab-content-container">
      ...
      <!-- H2R PANEL -->
      <div class="tab-panel" id="panel-h2r">
        <div class="form-group">
          <label>Paste H2R "Update" URL (Recommended)</label>
          <input type="text" id="h2r-pasted-url" value="${settings.h2rEndpoint}" placeholder="Right-click graphic -> Copy 'Update' URL and paste here..." />
          <p style="font-size:11px; color:var(--text-muted); margin:4px 0 0 0; line-height:1.4;">
            💡 <strong>Fastest Setup:</strong> In H2R Graphics, right-click your lower-third graphic, choose <strong>"Copy 'Update' URL"</strong>, and paste it above. The extension will automatically extract the project and graphic IDs!
          </p>
        </div>

        <div style="border-top:1px solid var(--border-color); margin:16px 0 8px 0; padding-top:12px; display:flex; flex-direction:column; gap:12px;">
          <div style="font-size:11px; font-weight:600; text-transform:uppercase; color:var(--text-secondary); letter-spacing:0.05em;">Parsed Connection Details</div>
          
          <div class="form-group">
            <label>H2R Host IP</label>
            <input type="text" id="h2r-host" value="${settings.h2rHost}" placeholder="http://localhost:4001" />
          </div>

          <div class="form-group">
            <label>H2R Project ID</label>
            <input type="text" id="h2r-project" value="${settings.h2rProject}" placeholder="default" />
          </div>

          <div class="form-group">
            <label>Lower-Third Graphic ID</label>
            <input type="text" id="h2r-graphic-id" value="${settings.h2rGraphicId}" placeholder="e.g. lower-third-abc123" />
            <p style="font-size:11px; color:var(--text-muted); margin:4px 0 0 0; line-height:1.4;">
              ⚠️ <strong>Crucial:</strong> H2R Graphics will crash with a <em>"TypeError: Cannot read properties of undefined (reading 'cues')"</em> if this Project ID or Graphic ID is incorrect. Using the Paste box above avoids this!
            </p>
          </div>
        </div>
        ...
        <div class="form-group">
          ...
          <div class="lower-third-preview">
            <div class="preview-label">Live Broadcast Preview</div>
            <div class="preview-graphic">
              <div class="preview-title" id="h2r-preview-t">Sermon Title | 講題</div>
              <div class="preview-subtitle" id="h2r-preview-s">Speaker & Hymns</div>
            </div>
          </div>
        </div>

        <div style="display:flex; align-items:center; gap:8px; margin:8px 0 16px 0;">
          <input type="checkbox" id="h2r-auto-show" ${settings.h2rAutoShow ? 'checked' : ''} style="width:16px; height:16px; margin:0;" />
          <label for="h2r-auto-show" style="font-size:12px; text-transform:none; letter-spacing:0; color:var(--text-primary); cursor:pointer;">Auto-Take Live (Show) on Push</label>
        </div>

        <button class="btn" id="tjc-push-h2r-btn">
          <svg style="width:18px;height:18px" fill="none" stroke="currentColor" viewBox="0 0 24 24">
  ...
  function updateH2RPreview() {
    ...
    const subtitleTemplate = parsedFields.speaker_zh 
      ? `講員/Speaker: ${parsedFields.speaker_zh} ${parsedFields.speaker_en} ｜ 詩歌/Hymns: ${hymnsList}` 
      : "Speaker & Hymns";
    
    const previewT = shadowRoot.querySelector('#h2r-preview-t');
    const previewS = shadowRoot.querySelector('#h2r-preview-s');
  ...
  async function pushToH2RGraphics() {
    ...
    const gid = shadowRoot.querySelector('#h2r-graphic-id').value.trim();
    const templateText = shadowRoot.querySelector('#h2r-payload-template').value;

    // Resilient endpoint resolver
    if (endpoint) {
      // Handle relative URIs like "graphic/5MMWQ/update" by prepending host and API context
      ...
      if (!host || !gid) {
        showToast("Please paste the H2R Update URL or specify the Host and Graphic ID.", "error");
        return;
      }
      if (proj) {
    ...
    const autoShow = shadowRoot.querySelector('#h2r-auto-show').checked;

    settings.h2rHost = host;
    settings.h2rProject = proj;
    settings.h2rGraphicId = gid;
    settings.h2rEndpoint = shadowRoot.querySelector('#h2r-pasted-url').value.trim();
    settings.h2rAutoShow = autoShow;
    ...
    try {
      ...
      if (response && response.success) {
        ...
        if (autoShow) {
          ...
          console.log("Auto-taking live (showing) graphic via URL:", showUrl);
          ...
          if (showResponse && showResponse.success) {
            showToast("Graphic is now live on screen!");
          }
        }
      } else {
        throw new Error(response ? response.error : "Failed to connect to H2R Graphics");
      }
    } catch (err) {
      console.error(err);
  ...
  const pastedUrlInput = shadowRoot.querySelector('#h2r-pasted-url');
  if (pastedUrlInput) {
    pastedUrlInput.addEventListener('input', async (e) => {
      const url = e.target.value.trim();
      const match = url.match(/^(https?:\/\/[^\/]+)\/api\/([^\/]+)\/graphic\/([^\/]+)/);
      if (match) {
        const host = match[1];
        const project = match[2];
        const graphicId = match[3];

        const hostInput = shadowRoot.querySelector('#h2r-host');
        const projectInput = shadowRoot.querySelector('#h2r-project');
        const graphicIdInput = shadowRoot.querySelector('#h2r-graphic-id');

        if (hostInput) hostInput.value = host;
        if (projectInput) projectInput.value = project;
        if (graphicIdInput) graphicIdInput.value = graphicId;

        // Save immediately to settings
        settings.h2rHost = host;
        settings.h2rProject = project;
        settings.h2rGraphicId = graphicId;
        await chrome.storage.local.set({ settings });

        showToast("Parsed and saved H2R connection parameters!");
        updateH2RPreview();
      }
    });
  }
